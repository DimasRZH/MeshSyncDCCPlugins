...
class MESHSYNC_PT_Scene(MESHSYNC_PT, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.operator("meshsync.send_selected_objects", text="Manual Sync (Selection only)")
...
class MESHSYNC_OT_AutoSync(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if not MESHSYNC_OT_AutoSync._timer:
            ...
            if not MESHSYNC_OT_AutoSync._registered:
                MESHSYNC_OT_AutoSync._generation += 1
                self._generation = MESHSYNC_OT_AutoSync._generation
    ...
    def modal(self, context, event):
        if getattr(self, "_generation", -1) != MESHSYNC_OT_AutoSync._generation:
            return {'FINISHED'}
        if event.type == "TIMER":
            self.update()
        return {'PASS_THROUGH'}
...
def register():
    ...
    if _is_registered:
        return
    if MESHSYNC_OT_AutoSync.load_handler not in bpy.app.handlers.load_post:
        bpy.app.handlers.load_post.append(MESHSYNC_OT_AutoSync.load_handler)
    for c in classes:
        bpy.utils.register_class(c)
    msb_initialize_properties()
    if msb_setBakingDefaults not in bpy.app.handlers.load_post:
    ...
    if on_depsgraph_update_post not in bpy.app.handlers.load_post:
...
def unregister():
    ...
    if not _is_registered:
        return
    if MESHSYNC_OT_AutoSync._timer:
        bpy.context.window_manager.event_timer_remove(MESHSYNC_OT_AutoSync._timer)
        MESHSYNC_OT_AutoSync._timer = None
    MESHSYNC_OT_AutoSync._generation += 1
    MESHSYNC_OT_AutoSync._registered = False
    bpy.context.scene.meshsync_auto_sync = False
    for handlers, handler in (
        (bpy.app.handlers.load_post, MESHSYNC_OT_AutoSync.load_handler),
        (bpy.app.handlers.load_post, msb_setBakingDefaults),
        (bpy.app.handlers.load_post, on_depsgraph_update_post),
        (bpy.app.handlers.depsgraph_update_post, on_depsgraph_update_post)):
        if handler in handlers:
            handlers.remove(handler)
    atexit.unregister(DestroyMeshSyncContext)
    msb_context.Destroy()
    for c in classes:
